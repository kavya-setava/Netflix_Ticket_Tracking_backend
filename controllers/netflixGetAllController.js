...
exports.updateTicketByKey_DB = async (req, res) => {
  try {
    const { ticketKey } = req.params;
    const { status, startTime, endTime, SLA, asap, backupEmail, previousEmail } = req.body;
    ...
    const isReassignment = previousEmail && previousEmail !== backupEmail;
    ...
    // Function to apply enable/disable logic for a CM
    const applyEnableDisableLogic = async (cmEmail, isCurrentTicketCM) => {
      const cmFilter = { backupCM_email: cmEmail };
      ...
      const asapTickets = allTickets.filter(ticket => {
        // If this is the ticket being updated and belongs to this CM, use the new status
        if (isCurrentTicketCM && ticket.ticketKey === ticketKey.trim()) {
      ...
      if (startTicket) {
        ...
        bulkOps.push({
          updateOne: {
            filter: { ticketKey: startTicket.ticketKey, backupCM_email: cmEmail },
            update: { $set: { enable: true } }
          }
        });
      }
      // CASE 2: No Start ticket - handle based on previous state and ASAP tickets
        ...
        // Check if we're transitioning FROM Start TO On Hold (only for current ticket's CM)
        const isStartToOnHold = isCurrentTicketCM && 
                                existingTicket.status === "Start" && 
        ...
        if (isStartToOnHold) {
          ...
          if (asapTickets.length > 0) {
            ...
            bulkOps.push({
              updateOne: {
                filter: { ticketKey: ticketKey.trim(), backupCM_email: cmEmail },
                update: { $set: { enable: true } }
              }
            });
          }
    ...
    // Apply logic for both CMs if reassignment, otherwise just current CM
    if (isReassignment) {
      // IMPORTANT: Apply logic for new CM first (ticket will be in their list after update)
      // This ensures the reassigned ticket gets the updated status and asap values
      await applyEnableDisableLogic(backupEmail, true);
      
      // Then apply logic for previous CM (ticket is now removed from their list)
      // This will re-evaluate their remaining tickets and enable appropriate ones
      // For example: if the only ASAP ticket was moved, their other tickets will now be enabled
      await applyEnableDisableLogic(previousEmail, false);
    } else {
      // Apply logic for current CM only
      await applyEnableDisableLogic(backupEmail, true);
    }
    ...
    return res.status(200).json({
      success: true,
      message: isReassignment ? "Ticket reassigned and updated successfully" : "Ticket updated successfully",
      data: updatedTicket
    });
  } catch (error) {
    console.error("⛔ DB update error:", error);
